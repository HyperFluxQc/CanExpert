...
| **test_expert/cdd.py** | `load_cdd()`: CANdela documents - VAR / DIAGCLASS (tmplref: DCLTMPL) / DIAGINST / SERVICE (tmplref: DCLSRVTMPL) -> PROTOCOLSERVICE REQ, whose CONSTCOMP and STATICCOMP (the instance's STATICVALUE for the template's SHSTATIC) give the request's constant bytes; STATEGROUPS (spec session/security) numbered from 1 for mayBeExec and trans; a DID's length from its instance's DATAOBJs, and its fields from their data types (CVALUETYPE bl/enc/qty, TEXTTBL's TEXTMAPs, LINCOMP's COMPs with their factor and offset, PVALUETYPE's unit). |
| **test_expert/odx.py** | `load_odx()` through odxtools (`coded_const_prefix()`, pre-condition states, state transitions, state charts by semantic; `did_fields()` from a ReadDataByIdentifier's positive response: byte and bit position, coded type, TEXTTABLE or LINEAR compu method, internal limits, unit) and `load_description()` by extension. |
