...
| **main_session.py** | `Session`: Connect/Disconnect and `dispatch_frame()`, the one path every frame takes - history, recording, then the `on_frame()` of every open tool window -; `send_can_message()` can be called from any thread (the transmit thread): it sends under `send_lock` and emits `MainWindow.frame_sent`, which reaches `dispatch_frame()` in the window's thread; with the panel, the script, the status strip and the unsolicited responses. `insert_marker()` (Ctrl+M), `quick_marker()` and `add_marker()` - the one path of a marker, also from `api.marker()` and `t.marker()` - into `marker_history`, the recording, every window's `on_marker()` and the Log. |
| **can_bus.py** | `open_channel()`/`create_can_bus()`, `CanWorker` (the session's only bus reader, which also sends TesterPresent), `ReceiveMailbox` (bus facade for code off the GUI thread; every frame sent through it, the worker's TesterPresent and flow control are reported by `message_sent(time.time(), identifier, data, extended)`). With `unsolicited=True` (the session's worker, unless the channel is listen-only) the worker emits `unsolicited(time, payload)` for the diagnostic responses no request waits for: between exchanges its `UnsolicitedAssembler` puts the response ID's frames together and answers a first frame with the session's flow control; during an exchange the mailbox's `uds_request()` hands over the replies that are not its answer, and when the exchange ends the mailbox hands back the frames it left unread (`push()` tells the worker whether an exchange was running, under the same lock as the count, so no frame is read twice or by no one). The answers to the session's own TesterPresent are not reported (`is_tester_present_answer()`). |
...
| **uds/dtc.py** | The DTC status byte of ISO 14229-1 annex D: `STATUS_BITS` and `status_text()` - the UDS Console's fault memory and the Dummy ECU's window share them. |
...
| **simulation_window.py** | Simulated nodes: a branch per `message.senders` entry with its messages, the data editable signal by signal, sent at their cycle times by a `CyclicSender` (`_sync_cyclic()`: the ticked messages while Start is pressed; a failing one stops the simulation), the counters and measured cycles shown by `refresh()`. What was ticked is remembered. The *Simulated nodes* tab of the Transmit window. |
| **cyclic.py** | `CyclicSchedule`: when each key of a set is next due - at `t + cycle`, not "now + cycle", without a burst after a long gap. `CyclicSender`: a thread of its own at a raised priority that sends each key's frame (asked for at every send, so an edit goes out with the next) at its cycle, waiting with `timing.Waiter`; a send that fails stops its key (`failed(key, error)`); `take(key)` gives the sends since the last call and `CycleStats.text()`, the measured cycle over the last 100 sends. |
| **timing.py** | Waiting to the fraction of a millisecond: `Waiter.wait_until()` sleeps on a Windows high-resolution waitable timer (`CREATE_WAITABLE_TIMER_HIGH_RESOLUTION`, through ctypes; an ordinary timer before Windows 10 1803, `threading.Event` elsewhere) until half a millisecond before the moment and spins the rest; `wake()` ends it early (a timer and an event, `WaitForMultipleObjects`). `precise_switching()` makes the interpreter change threads every 0.5 ms instead of 5, `raise_priority()` raises the calling thread's. Measured on a development PC for a 10 ms cycle: `time.sleep()` 15.8 ms on average (10.3-25.5), the waitable timer 10.00 (9.3-10.6), with the spin 10.00 (9.7-10.3). |
| **transmit_window.py** | The transmit list: rows (raw or bound to a database message) in a table, the ticked ones sent by a `CyclicSender` (`_sync_cyclic()`: each row's frame read at every send), `refresh()` showing the counters and the measured cycles, `SignalEditor` re-encodes a message signal by signal, rows stored as JSON in the settings or a file. A row that fails to send switches itself off (`cyclic_failed`, from the sending thread). The *Messages* tab of the Transmit window. |
...
| **testing/report.py** | `html_report()` (one self-contained page: verdict, counts, facts - and more given by the caller - each case's steps, the ones that did not pass or have warnings or accepted deviations opened, and sections given by the caller), `summary_text()`, `junit_report()` (one testsuite; failed setup/teardown become testcases so CI shows why everything was skipped; a blocked case is an error of type Blocked) and `save_reports()` (`<module>_<date-time>.html/.xml`, or a stem of the caller's: a repeated run's number). |
...
| **test_expert/description.py** | TestExpert's model of an ECU's diagnostics: `EcuDescription` with `Session`s (and where each is entered from, `session_path()`), what it does not know (`unknown`: writing, sub-functions, starting routines), each DID's `DataField`s (position, bits, encoding, valid coded ranges, text table, scale, shift, unit; `coded()`, `encode()`, `check()`, `shown()`), security levels, `Service`s with their sub-functions, `DataIdentifier`s (with their InputOutputControl, `io`, and its control parameters where the file names them) and `Routine`s, each with an `Access` (sessions, security levels), the ECU's DTCs (`dtcs`: trouble code -> text); JSON; `build_description()` from `RawService`s - the constant bytes a request starts with, the states it may be executed in, the transitions it causes - and `RawState`s: sessions from DiagnosticSessionControl's transitions into session states, levels from SecurityAccess's into security states, a service needing a level when only unlocked states allow it. |
| **test_expert/cdd.py** | `load_cdd()`: CANdela documents - VAR / DIAGCLASS (tmplref: DCLTMPL) / DIAGINST / SERVICE (tmplref: DCLSRVTMPL) -> PROTOCOLSERVICE REQ, whose CONSTCOMP and STATICCOMP (the instance's STATICVALUE for the template's SHSTATIC) give the request's constant bytes; STATEGROUPS (spec session/security, else their name) numbered from 1 for mayBeExec and trans (a warning for documents without them); a DID's data record from its instance's data container - a SIMPLECOMPCONT whose SHPROXY is the data, not the response codes - walked through DATAOBJs, GAPDATAOBJs, STRUCTs and UNIONs (`record()`), each data object's type (CVALUETYPE bl/enc/qty, TEXTTBL's TEXTMAPs, LINCOMP's COMPs with factor, offset and divisor, PVALUETYPE's UNIT); a warning for KWP2000 documents; the variants (VARs) and the one to read (the first by default; `CddError` for
one the file does not have). Checked against a real CANdelaStudio export (cantools' example.cdd). |
| **test_expert/odx.py** | `load_odx()` through odxtools (`coded_const_prefix()`, pre-condition states and state transitions matched by the states' ODX IDs, state charts by semantic, names without CANdelaStudio's _Read/_Write/_Start endings; `did_fields()` from a ReadDataByIdentifier's positive response: byte and bit position, coded type, TEXTTABLE or LINEAR compu method, internal limits, unit; a MIN-MAX-LENGTH-TYPE at the end as text of any length), the DTCs of the layer's DTC-DOPs (`odx_services.dtc_texts()`), the variant to read
...
| **test_expert/services.py** | `ServiceTests`: services taken further than their availability - download and upload (out of order, locked, a wrong format; the plan's `download` started, then refused again and ended by the default session), memory by address (a wrong format; the plan's `memory` read, written while locked and written back), periodic data (a DID of F200-F2FF sent and stopped, watched through `transport.Link`), ResponseOnEvent (no event; an event on a DID found changing between two reads), CommunicationControl stopping the ECU's own frames (counted by identifier besides the diagnostic ones), InputOutputControl of the DIDs that have it (00; destructive, 03 to the value read, then 00) and of one that has none, the ECU's DTCs (19 0A, 19 02 FF) against the description's - a 3-byte code, or its 2 bytes without the failure type; `parse_memory_range()`. Their NRCs are situations of the NRC policy. |
...
the bus is opened), how many times to run (`repeat`, `until_failure`), the test modules and symbol databases (`module_paths()`, `symbol_paths()`; moved
...
opening the bus, exit code 2 when it cannot be told), `--module` and `--symbols` (added to the plan's), `--test` (a test or group, `selected()`), `--repeat` and `--until-failure` (the exit code 1 when any run did not pass, the JUnit copy of the first such run), the verdicts printed, the reports (and a copy of the JUnit XML with `--junit`), exit code 0/1/2; the frozen windowed program attaches to its parent's console. |
...
| **test_expert/tester.py** | `Tester.ask()`/`quiet()`: a request, physical or functional, and the timed `Answer` (positive, NRC, text, the time to its first frame and to each response pending - on `time.perf_counter()`, so P2 is checked to the millisecond -, busy repeats, a transport error); the session the ECU is in, followed from its answers; `send_frame()` for sequences. |
| **test_expert/window.py** | TestExpert's window: a toolbar of `QAction`s (`TOOLBAR`, the symbols of `ui_common.toolbar_icon()` in the theme's colours) shared by the menus and the ECU tab, each run compared with the last one of the same description (the Comparison tab, shown when something regressed; any two runs from the File menu), what it holds is a test plan (`plan()`, `apply_plan()`; new, open, save, save as; kept in the settings with absolute paths for the next start), the description (open, Dummy ECU, save as JSON; its variants: chosen, identified - on connecting too), the ECU connection (a CAN worker without TesterPresent, identifiers from a CAN Expert configuration), the settings, the sequences, the tests to tick (with the sequences around each, and a menu to attach them), the run on a thread with live verdicts, reports and an optional `.blf` recording in `TestExpert/reports` - run control: a test or a group run from the tests' menu, the failed ones again (`run_failed()`, `NOT_PASSED`), runs repeated (`run(names, repeat, until_failure)`: each run started by the one before as it ends, `_next_run()`, with its own reports), a progress bar counting the tests done, the tree filtered by words and to what did not pass (`apply_filter()`); `main()`. |
...
- `tests/test_recording_and_workspace.py`: the frame history with the adapter's timestamps, a frame sent from another thread reaching the windows from theirs with its extended identifier, the session's own frames keeping their moment and extended identifier, the ECU check feeding the Trace, a window opened later filled from the history, recording to a file and replaying it offline, the workspace windows (tabbing, floating, the toolbar switches) and the saved layout and desktops.
...
- `tests/test_transmit_window.py`: editing rows, rejecting bad input, a database message and its signal editor, sending once and cyclically - by the sending thread, an edit going out with the next send, the measured cycle, every send counted -, a failing row switching itself off, and the list surviving a restart.
- `tests/test_timing.py`: `Waiter` (on the moment and never before, woken by another thread, a moment passed, the high-resolution timer on Windows, the switch interval) and `CyclicSender` (frames at their cycle, the frame asked for at each send, None skipping a send, a failing key stopped alone, no burst after a stall, keys coming and going, closed for good), `CycleStats`.
...
- `tests/test_simulation.py`: `CyclicSchedule` (due times, a late tick, a long gap, independent keys) and the simulated nodes - the nodes a database gives, ticking a message or a whole node (sent by the sending thread at the measured cycle, nothing before Start or after Stop), the signal editor, a failing send stopping the simulation, what is remembered - and the Transmit window holding both tabs.
