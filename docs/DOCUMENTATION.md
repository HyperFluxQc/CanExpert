...
| **test_expert/engine.py** | `PlanRun`: one run of a plan on a bus or a CAN worker's mailbox - the suite (with the plan's NRC policy), the tester, the runner (with its accepted deviations) - with `run()`, `stop()`, `save()` (the reports, with the plan and the description in their facts); `RecordingBus` records a bus's frames both ways for runs without the window. |
