...
sub-functions and the sessions and security level it needs, every DID with its length and where it may be
read and written, and the routines. What the file did not say clearly is listed under **Warnings**.
...
first variant is read. `ODX/dummy_ecu.cdd` describes the Dummy ECU in that structure (it is generated by
...
| Data identifiers | every DID read in every session — positive with its length where allowed, 0x31 where not, 0x33 while locked and positive once unlocked; a DID that does not exist; writing a read-only DID (0x31) |
...
| Routines | each routine refused where it may not run (0x31) and while locked (0x33); a routine that does not exist |
...
| Timing | responses within the P2 the ECU announces, plus the margin |

On the **Settings** tab:

- **Destructive tests** adds what changes the ECU: ECU reset (and the default session after it), clearing
  every DTC, and writing each writable DID — its own value, read first, written back.
