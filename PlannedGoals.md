...
| **TestExpert: real files, variants, test modules** — **DONE** | medium | The CDD and ODX readers checked against real files (a CANdelaStudio export, odxtools' example PDX): data containers, divisors and units, unions, gaps, BCD, text of any length, KWP2000 documents warned about; a file's variants chosen, or identified on the ECU (ODX ECU-VARIANT-PATTERNs through odxtools' matcher, a DID for CDD files), in the window, plans and the command line; CAN Expert's test modules run in the same run as groups, with their hooks, frames and symbol databases |
