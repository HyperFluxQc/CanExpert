...
| **TestExpert: real files, variants, test modules, transport layer, run control** — **DONE** | medium | The CDD and ODX readers checked against real files (a CANdelaStudio export, odxtools' example PDX): data containers, divisors and units, unions, gaps, BCD, text of any length, KWP2000 documents warned about; a file's variants chosen, or identified on the ECU (ODX ECU-VARIANT-PATTERNs through odxtools' matcher, a DID for CDD files), in the window, plans and the command line; CAN Expert's test modules run in the same run as groups, with their hooks, frames and symbol databases; the ISO-TP transport layer tested both ways (it found that the Dummy ECU kept a segmented request going after a single frame; fixed); download, memory, periodic data, ResponseOnEvent, CommunicationControl's effect, IO control and the ECU's DTCs against the description's tested; security access further - seeds, key length, levels, the lockout across a reset (the Dummy ECU now answers a key of the wrong length with 0x13); run control - a test or group run from its menu, the failed tests again, runs repeated (until one fails), progress, a filter, and --test, --repeat on the command line (it found the Dummy ECU's description saying WriteMemoryByAddress needs no security; fixed) |
| **Cyclic messages on time** — **DONE** | medium | The Transmit window's rows, the simulated nodes and the Dummy ECU's application frames are sent by threads of their own waiting on Windows' high-resolution timer (a 10 ms message went every 15.8 ms on average, 10.3 to 25.5; now 10.00, within about half a millisecond), with a measured-cycle column; frames sent from other threads keep their time and extended identifier in the trace; TestExpert times answers to the millisecond. Exact to the microsecond would need the adapter to send (Kvaser's periodic object buffers, Professional devices only) |
